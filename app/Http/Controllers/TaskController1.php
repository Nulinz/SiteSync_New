...
class TaskController extends Controller
{
    ...
    //     $tasks = Task::with(['project:id,project_name', 'user:id,name'])->where('created_by', auth()->user()->id)->whereColumn('parent_task_id', 'id')->get();
    ...
    //         // Fetch data from all tables based on the condition 'c_on'
    //         $table1Data = Task::with(['created_user:id,name,role_id', 'created_user.role:id,name', 'project:id,project_name,project_id'])->where('assigned_to', auth()->user()->id)->get()->map(function ($item) {
    ...
    //             $item->setAttribute('cby_det', $item->created_user->name ?? null); // Accessing user_cby's name
    //             $item->setAttribute('cby_role', $item->created_user->role->name ?? null); // Accessing user_cby's role_title
    //             $item->setAttribute('file_attachment', $item->file_attachment ? asset('img/task/' . $item->file_attachment) : null);
    ...
    //         $table2Data = EntrySurvey::with(['user_cby:id,name,role_id', 'user_cby.role:id,name', 'survey:id,title', 'project:id,project_name,project_id', 'survey_ans:entry_ans'])->where('assigned_to', auth()->user()->id)->get()->map(function ($item) {
    //             // Set custom attributes
    //             $item->setAttribute('source_table', 'ent_survey');

    //             // Set the user's name and role title from the user_cby relationship
    //             $item->setAttribute('survey_title', $item->survey->title ?? null); // Accessing user_cby's name
    //             $item->setAttribute('project_title', $item->project->project_name ?? null); // Accessing user_cby's name
    //             $item->setAttribute('cby_det', $item->user_cby->name ?? null); // Accessing user_cby's name
    //             $item->setAttribute('cby_role', $item->user_cby->role->name ?? null); // Accessing user_cby's role_title
    //             $item->setAttribute('file_attachment', $item->file_attachment ? asset('img/survey/' . $item->file_attachment) : null);
    //             $item->setAttribute('ans_row', $item->survey_ans->count());
    ...
    //             unset($item->survey);
    //             unset($item->project);
    //             unset($item->survey_ans);
    //             return $item;
    //         });

    //         $table3Data = EntryQC::with(['user_cby:id,name,role_id', 'user_cby.role:id,name', 'qc:id,title', 'project:id,project_name,project_id', 'qc_ans:qc_entry'])->where('assigned_to', auth()->user()->id)->get()->map(function ($item) {
    //             $item->setAttribute('source_table', 'ent_qc');

    //             // Set the user's name and role title from the user_cby relationship
    //             $item->setAttribute('qc_name', $item->qc->title ?? null); // Accessing user_cby's name
    //             $item->setAttribute('project_title', $item->project->project_name ?? null); // Accessing user_cby's name
    //             $item->setAttribute('cby_det', $item->user_cby->name ?? null); // Accessing user_cby's name
    //             $item->setAttribute('cby_role', $item->user_cby->role->name ?? null); // Accessing user_cby's role_title
    //             $item->setAttribute('file_attachment', $item->file_attachment ? asset('img/qc/' . $item->file_attachment) : null);
    //             $item->setAttribute('ans_row', $item->qc_ans->count());
    //             $item->setAttribute('c_at', $item->created_at ? $item->created_at : null);
    //             $item->setAttribute('pro_code', optional($item->project)->project_id);
    ...
    //         $table4Data = EntrySnag::with(['user_cby:id,name,role_id', 'user_cby.role:id,name', 'snag:id,category', 'project:id,project_name,project_id', 'snag_ans:entry_snag'])->where('assigned_to', auth()->user()->id)->get()->map(function ($item) {
    //             $item->setAttribute('source_table', 'ent_snag');

    //             // Set the user's name and role title from the user_cby relationship
    //             $item->setAttribute('qc_name', $item->snag->category ?? null); // Accessing user_cby's name
    //             $item->setAttribute('project_title', $item->project->project_name ?? null); // Accessing user_cby's name
    //             $item->setAttribute('cby_det', $item->user_cby->name ?? null); // Accessing user_cby's name
    //             $item->setAttribute('cby_role', $item->user_cby->role->name ?? null); // Accessing user_cby's role_title
    //             $item->setAttribute('ans_row', $item->snag_ans->count());
    //             $item->setAttribute('file_attachment', $item->file_attachment ? asset('img/snag/' . $item->file_attachment) : null);
    //             $item->setAttribute('c_at', $item->created_at ? $item->created_at : null);
    //             $item->setAttribute('pro_code', optional($item->project)->project_id);
    ...
    //         // // Merge all the data into a single collection
    //         $allData = collect($table1Data)->merge(collect($table2Data))
    //             ->merge(collect($table3Data))
    //             ->merge(collect($table4Data));

    //         // Sort the merged data by a specific column, for example, 'created_at'
    //         // $sortedData = $allData->sortByDesc('created_at')->except('created_at', 'updated_at'); // Or any other column you want to sort by
    ...
    //             // // // If the item is an object (like an Eloquent model)
    //             // if (is_object($item)) {
    ...
    //         // Split the data based on the 'status' field
...
//     // For web requests, get data for all tabs
...
//         ->get()
//         ->map(function ($task) {
//             // Check if this task has any close requests - get full close request object
//             $closeRequest = DB::table('task_close')->where('request_to_task', $task->id)->first();
//             $task->close_request = $closeRequest;
//             return $task;
//         });

//     // Assigned Tasks (tasks assigned to the logged-in user by others)
...
// {
//     $request->validate([
//         'request_by_task' => 'required|integer',
//     ]);

//     $taskId = $request->input('request_by_task'); // works with JSON, form-data, or x-www-form-urlencoded

//     $updated = DB::table('task_close')
//         ->where('request_by_task', $taskId)
...
//                 'file' => $taskClose->file ? asset('img/task/' . $taskClose->file) : null,
...
// public function getTaskCloseDetails($taskId)
// {
//     $taskClose = DB::table('task_close')
//         ->where('request_by_task', $taskId)
//         ->select('id', 'remarks', 'file', 'created_at')
...
public function store(Request $request)
{
    ...
    $data = [
       ...
       'file_attachment'=> $filePath, // Store only the filename
    ...
    public function task_status_update(Request $req)
    {
        ...
        if ($table != 'task') {
            ...
            // Dynamically access the title
            // if (is_array($title_column)) {
            //     // For cases like 'ent_qc' where we have ['qc', 'title']
            //     $title_col = $task->{$title_column[0]}->{$title_column[1]};
            //     $cr_by = 'user_cby';
            // } else {
            //     // For simple cases like 'task' or 'ent_survey'
            //     $title_col = $task->{$title_column};
            //     $cr_by = 'created_user';
            // }
...
//         ->where('created_by', auth()->user()->id)
//         // Fixed: This condition was likely incorrect - should probably be:
//         // ->whereNull('parent_task_id') // for root tasks, or
//         // ->where('parent_task_id', '!=', DB::raw('id')) // if you want to exclude self-referencing
//         ->orderBy('created_at', 'desc')
//         ->get()
//         ->map(function ($lt) {
//             // Format created_at
...
//             // File attachment full path
//             $lt->setAttribute('file_attachment', $lt->file_attachment ? asset('img/task/' . $lt->file_attachment) : null);
...
//                     // This handles the case where task.status != 'pending' but task_close.status == 'pending'
//                     $task_status = 'Pending';
//                 }
...
//             // ✅ Priority 1: Approved in task_close
//             if ($taskClose && $taskClose->status == 'approved') {
//                 $task_status = 'Approved';

//             // ✅ Priority 2: Completed in task table
//             } elseif ($lt->status === 'completed') {
//                 $task_status = 'Completed';

//             // ✅ Priority 3: Other cases
//             } elseif ($taskClose) {
//                 if ($taskClose->status == 'completed' || $taskClose->status == 'closed') {
//                     $task_status = 'Completed';
//                 } elseif ($taskClose->status == 'pending') {
...
//                     $task_status = 'Approved';
//                 } elseif ($taskClose->status == 'completed' || $taskClose->status == 'closed') {
//                     $task_status = 'Completed';
//                 }
//             } else {
//                 // If not closed → check date
...
public function assign_to_me(Request $req)
{
    $tasks = Task::with(['project:id,project_name', 'user:id,name'])
        ...
        ->map(function ($lt) {
            ...
            // File path
            $lt->setAttribute('file_attachment', 
                $lt->file_attachment 
                    ? env('AWS_URL') . $lt->file_attachment 
                    : null
            );
